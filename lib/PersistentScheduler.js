      ...
      */
      ...
      return this.store.getOneTimeEvents(scheduleInterval, function(err, oneTimeEvents) {
        ...
        return _this.store.getUnsolvedEvents(function(err, unsolvedOneTimeEvents) {
          var event, id, payload, timestamp, todoEvents, _i, _len, _ref, _results;
          if (err) {
            throw new util.Error(err);
          }
          ...
          for (_i = 0, _len = todoEvents.length; _i < _len; _i++) {
            _ref = todoEvents[_i], id = _ref.id, timestamp = _ref.timestamp, event = _ref.event, payload = _ref.payload;
            _this.schedule(timestamp, event, payload);
            _results.push(_this.store.remove(id, function() {}));
          }
          return _results;
        });
      });
